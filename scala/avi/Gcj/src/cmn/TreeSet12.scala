 ...
 *
 */
object TreeSet12 extends MutableSortedSetFactory[TreeSet12] {
  /**
   *  The empty set of this type
   */
  def empty[A](implicit ordering: Ordering[A]) = new TreeSet12[A]()

  /** $sortedMapCanBuildFromInfo */
  implicit def canBuildFrom[A](implicit ord: Ordering[A]): CanBuildFrom[Coll, A, TreeSet12[A]] =
...
sealed class TreeSet12[A] private(tree: RedBlackTree.Tree[A, Null])(implicit val ordering: Ordering[A])
  extends AbstractSortedSet[A]
  with SortedSet[A]
  with SetLike[A, TreeSet12[A]]
    with SortedSetLike[A, TreeSet12[A]]
    with Serializable {
  ...
  override def empty = TreeSet12.empty
  override protected[this] def newBuilder = TreeSet12.newBuilder[A]
   ...
   *              bound.
   */
  def rangeImpl(from: Option[A], until: Option[A]): TreeSet12[A] = new TreeSetView(from, until)
  ...
  /**
   * A ranged projection of a [[TreeSet12]]. Mutations on this set affect the original set and vice versa.
   *
   * Only keys between this projection's key range will ever appear as elements of this set, independently of whether
   * the elements are added through the original set or through this view. That means that if one inserts an element in
   * a view whose key is outside the view's bounds, calls to `contains` will _not_ consider the newly added element.
  ...
  private[this] final class TreeSetView(from: Option[A], until: Option[A]) extends TreeSet12[A](tree) {
    ...
    override def rangeImpl(from: Option[A], until: Option[A]): TreeSet12[A] =
