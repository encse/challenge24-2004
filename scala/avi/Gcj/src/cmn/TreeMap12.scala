 ...
 */
object TreeMap12 extends MutableSortedMapFactory[TreeMap12] {

  def empty[A, B](implicit ord: Ordering[A]) = new TreeMap12[A, B]()(ord)

  /** $sortedMapCanBuildFromInfo */
  implicit def canBuildFrom[A, B](implicit ord: Ordering[A]): CanBuildFrom[Coll, (A, B), TreeMap12[A, B]] =
...
sealed class TreeMap12[A, B] private(tree: RedBlackTree.Tree[A, B])(implicit val ordering: Ordering[A])
  extends AbstractSortedMap[A, B]
  with SortedMap[A, B]
  with MapLike[A, B, TreeMap12[A, B]]
  with SortedMapLike[A, B, TreeMap12[A, B]]
  with Serializable {
  ...
  override def empty = TreeMap12.empty
  override protected[this] def newBuilder = TreeMap12.newBuilder[A, B]
   ...
   *              bound.
   */
  def rangeImpl(from: Option[A], until: Option[A]): TreeMap12[A, B] = new TreeMapView(from, until)
  ...
  /**
   * A ranged projection of a [[TreeMap12]]. Mutations on this map affect the original map and vice versa.
   *
   * Only entries with keys between this projection's key range will ever appear as elements of this map, independently
   * of whether the entries are added through the original map or through this view. That means that if one inserts a
   * key-value in a view whose key is outside the view's bounds, calls to `get` or `contains` will _not_ consider the
  ...
  private[this] final class TreeMapView(from: Option[A], until: Option[A]) extends TreeMap12[A, B](tree) {
    ...
    override def rangeImpl(from: Option[A], until: Option[A]): TreeMap12[A, B] =
