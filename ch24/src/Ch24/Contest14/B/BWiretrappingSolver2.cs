...
{
    public class BWiretrappingSolver2 : Contest.Solver
    {
        public class Node
        {
            public int i;
        ...
        public override void Solve()
        {
            ...
            Save(mpnodeByI, FpatIn + ".orig.csv");
            var removed = removeE(mpnodeByI, ia, ib);
            Save(removed, FpatIn + ".removed.csv");
            //var merged = merge(mpnodeByI, ia, ib);
            //Save(merged, FpatIn + ".merged.csv");
        ...
        private void Save(Dictionary<int, Node> mpnodeByI, string filn)
        {
            using(var fs = new FileStream(filn, FileMode.Create))
            using(var sw = new StreamWriter(fs))
            {
                var rgi = mpnodeByI.Keys.ToList();

                for(int y = 0; y < rgi.Count-1; y++)
                {
                    for(int x = 0; x < rgi.Count-1; x++)
                    {
                        if(x>0)
                            sw.Write(", ");
                        if(x==y)
                            sw.Write(mpnodeByI[rgi[x]].rgi.Count);
                        else
                            sw.Write(mpnodeByI[rgi[x]].rgi.Contains(rgi[y]) ? -1 : 0);
                    }
                    sw.WriteLine();
                }
                sw.Flush();
